    ...
    def forward(self, x, augment=False, profile=False):
        if augment:
            ...
            for i, xi in enumerate((x,
                                    torch_utils.scale_img(x.flip(3), s[0]),  # 在第三个维度翻转
            ...
            y[2][..., :4] /= s[1]
            return torch.cat(y, 1), None  # 图像增强推理，训练
        else:
...
def parse_model(md, ch):
    ...
    for i, (f, n, m, args) in enumerate(md['backbone'] + md['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, DWConv, Bottleneck, SPP, DWConv, MixConv2d, Focus, ConvPlus, BottleneckCSP, GhostBottleneck, GhostBottleneckAT]:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5s-ghost-eca.yaml', help='model.yaml')
